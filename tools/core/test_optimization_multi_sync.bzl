...
def _record_multi_repo_owner_or_fail(seen_repo_owners, repo_name, owner):
    ...
    if prev_owner != None:
        fail_with_prefix(
            "test_optimization_multi_sync",
            "duplicate repository name '%s' generated by modules '%s' and '%s'. Use unique `name` values across test_optimization_multi_sync tags." %
...
def _render_aggregate_bzl(service_keys, repo_names):
    ...
    if len(service_keys) != len(repo_names):
        ...
        fail_with_prefix("test_optimization_multi_aggregate", "service_keys and repo_names length mismatch")
    entries = []
...
def _multi_aggregate_impl(ctx):
    ...
    build_content = (
        ...
        "define_targets()\n\n" +
        'exports_files(["export.bzl"], visibility = ["//visibility:public"])\n'
    )
    ...
    for mod in module_ctx.modules:
        ...
        for call in mod.tags.test_optimization_multi_sync:
            ...
            if not services:
                ...
                fail_with_prefix("test_optimization_multi_sync", "'services' must be a non-empty list")
