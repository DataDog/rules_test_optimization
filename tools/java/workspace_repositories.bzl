...
"""WORKSPACE repository helpers for Datadog Java Test Optimization onboarding.

This helper wires only the Datadog Java companion module. Java rules,
toolchains, test framework dependencies, and the dd-java-agent artifact remain
consumer-owned so repositories keep their existing dependency policy.
"""
...
_OWNER = "datadog_java_test_optimization_workspace_repositories"
_JAVA_COMPANION_REPO = "datadog-rules-test-optimization-java"
_CORE_REPO = "datadog-rules-test-optimization"
...
def _fail_existing_repo(name):
    ...
    fail_with_prefix(
        ...
        (
            ...
            "for '%s', or stop calling this helper for the Java companion repository. " +
            "If the collision is with a consumer-owned rules_java repository, keep " +
            "that repository and pass its name through rules_java_repo_name. " +
            "Example:\n\n" +
...
def _fail_missing_repo(name, repo_kind):
    """Fail when the consumer has not declared a required mapped repo."""
    fail_with_prefix(
        _OWNER,
        (
            "%s repository '%s' is not declared. Declare %s before calling " +
            "this helper, or pass the consumer-owned repository name. Example:\n\n" +
            "datadog_java_test_optimization_workspace_repositories(\n" +
            "    rto_commit = \"<published-sha>\",\n" +
            "    rules_java_repo_name = \"%s\",\n" +
            ")\n"
        ) % (repo_kind, name, repo_kind, name),
    )

def _validate_fetch_mode(value):
    """Validate the Java companion repository fetch mode."""
...
def _require_archive_attrs(url, sha256, prefix, archive_type):
    """Validate archive-mode inputs for the Datadog Java companion archive."""
    missing = []
    if not url:
...
def _build_java_workspace_repository_specs(
        rto_commit,
        rto_remote = "https://github.com/DataDog/rules_test_optimization.git",
        datadog_fetch = "git",
        rules_java_repo_name = "rules_java",
        core_repo_name = _CORE_REPO,
        rto_archive_url = None,
        rto_archive_sha256 = None,
        rto_archive_prefix = None,
        rto_archive_type = "tar.gz",
        existing_repositories = None):
    """Build normalized repository specs for the Java WORKSPACE helper.

    Args:
      rto_commit: Published commit to use for git_repository fetches.
      rto_remote: Git remote for Datadog repositories.
      datadog_fetch: Fetch mode for the Java companion repository.
      rules_java_repo_name: Repository name used by the consumer for rules_java.
      core_repo_name: Repository name used by the consumer for the core Datadog repository.
    ...
    if not rules_java_repo_name:
        fail_with_prefix(_OWNER, "rules_java_repo_name must be non-empty")
    if not core_repo_name:
        fail_with_prefix(_OWNER, "core_repo_name must be non-empty")
    ...
    if existing_repositories != None:
        if _JAVA_COMPANION_REPO in existing_repositories:
            _fail_existing_repo(_JAVA_COMPANION_REPO)
        if core_repo_name not in existing_repositories:
            _fail_missing_repo(core_repo_name, "core Datadog")
        if rules_java_repo_name not in existing_repositories:
            _fail_missing_repo(rules_java_repo_name, "rules_java")

    companion_repo_mapping = {
        "@datadog-rules-test-optimization": "@" + core_repo_name,
        "@rules_java": "@" + rules_java_repo_name,
    }
    if datadog_fetch == "git":
        return [{
            "kind": "git_repository",
            "attrs": _git_attrs(
                _JAVA_COMPANION_REPO,
                rto_remote,
                rto_commit,
                "modules/java",
    ...
    return [{
        ...
        "attrs": _archive_attrs(
            ...
            rto_archive_url,
            rto_archive_sha256,
            rto_archive_prefix + "/modules/java",
            rto_archive_type,
            repo_mapping = companion_repo_mapping,
        ),
    }]
...
def datadog_java_test_optimization_workspace_repositories(
        rto_commit,
        rto_remote = "https://github.com/DataDog/rules_test_optimization.git",
        datadog_fetch = "git",
        rules_java_repo_name = "rules_java",
        ...
        rto_archive_prefix = None,
        rto_archive_type = "tar.gz"):
    """Declare Java Test Optimization repositories for WORKSPACE consumers.

    The consumer must declare `datadog-rules-test-optimization` and
    `rules_java` before loading this helper. This helper wires only the Java
    companion repository.
    """
    specs = _build_java_workspace_repository_specs(
        rto_commit = rto_commit,
        rto_remote = rto_remote,
        datadog_fetch = datadog_fetch,
        rules_java_repo_name = rules_java_repo_name,
        core_repo_name = core_repo_name,
...
build_java_workspace_repository_specs_for_tests = _build_java_workspace_repository_specs
