...
def _impl(ctx):
    ...
    go_bzl = (
        "# Generated by test_optimization_sync; provides GO_MODULE_PATH if detected\n"
        + "GO_MODULE_PATH = \"%s\"\n" % (go_module_path or "")
        + "SANITIZED_GO_MODULE_PATH = \"%s\"\n" % sanitized_go_module_path
    )
    ctx.file("go_module.bzl", go_bzl)
    ...
    index_bzl = (
        "# Generated by test_optimization_sync; sanitized module labels present in this repo\n"
        + "SANITIZED_MODULE_LABELS = %s\n" % repr(labels)
        + "SANITIZED_MODULE_SET = %s\n" % set_literal
    )
    ctx.file("modules_index.bzl", index_bzl)
    ...
    build_content = (
        ...
        + '    visibility = ["//visibility:public"],\n'
        + ')\n'
        + '\nexports_files(["go_module.bzl", "modules_index.bzl"])\n'
    )
