...
`--no-ddtrace`. The target must provide `pytest` and `ddtrace` through its
normal Python dependency repository; replace `@python_deps` with the repository
name generated by your `rules_python` `pip_parse` / `pip.parse` setup.

For `unittest` or another custom runner, pass `main`, `py_test_rule`, and any
custom `imports` or `args` you need. In that mode the macro still wires the
manifest, payload-file mode, per-module selection data, and Bazel target
metadata, but the custom runner is responsible for activating the Python
tracer.
