...
- the retry matrix explicitly covers transient connection and timeout recovery,
  HTTP `408`, `429`, `500`, `502`, `503`, and `504`, terminal `400`, `401`,
  `403`, `404`, and `413`, and byte-identical gzip/multipart replay;
- split regression coverage proves a failed middle chunk retains its source and
  prevents later chunks, and a gated worker proves the bounded queue applies
...
- [x] Update README uploader prerequisites and examples.
- [x] Update `docs/Uploader_Reference.md` with worker and split semantics.
- [x] Update `docs/Uploader_Reference.md` with final statistics, debug, and
      dry-run behavior.
- [x] Update `docs/Configuration_Reference.md` with `workers`, `--debug`, and
      `--dry-run` controls.
- [x] Update troubleshooting for Python resolution, split failures, and `413`.
- [x] Document the normalized four-attempt retry budget and terminal `4xx`
      behavior as intentional migration changes.
- [x] Confirm contributor validation instructions need no update because the
      maintained validation entrypoints did not change.
- [x] Validate this repository first.
