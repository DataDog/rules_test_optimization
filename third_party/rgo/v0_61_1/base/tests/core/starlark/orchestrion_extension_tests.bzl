...
def _go_cache_selection_test(ctx):
    env = unittest.begin(ctx)

    base_env = orchestrion_extension_test_helpers.base_go_env(struct(
        attr = struct(go_sdk_root = ""),
        os = struct(environ = {}, name = "linux"),
    ))

    asserts.false(env, "GOCACHE" in base_env, "cache selection happens after the base environment is built")
    asserts.false(env, "GOMODCACHE" in base_env, "cache selection happens after the base environment is built")
    asserts.equals(env, "on", base_env["GO111MODULE"])
    ...
    standard = orchestrion_extension_test_helpers.select_go_cache_env(linux_ctx, parsed.gocache, parsed.gomodcache, "/repo/.orchestrion_bootstrap_go_cache", True, True)
    asserts.equals(env, parsed.gocache, standard["GOCACHE"])
    asserts.equals(env, parsed.gomodcache, standard["GOMODCACHE"])

    fallback = orchestrion_extension_test_helpers.select_go_cache_env(linux_ctx, parsed.gocache, parsed.gomodcache, "/repo/.orchestrion_bootstrap_go_cache", False, True)
    asserts.equals(env, "/repo/.orchestrion_bootstrap_go_cache/cache", fallback["GOCACHE"])
    asserts.equals(env, parsed.gomodcache, fallback["GOMODCACHE"])

    windows_fallback = orchestrion_extension_test_helpers.select_go_cache_env(struct(os = struct(name = "windows_nt")), "C:\\Users\\runner\\go-build", "C:\\Users\\runner\\go\\pkg\\mod", "C:\\repo\\.orchestrion_bootstrap_go_cache", True, False)
...
go_cache_selection_test = unittest.make(_go_cache_selection_test)
