...
def _go_infos_use_cgo(go_infos):
    """Return whether any GoInfo provider in the input list uses cgo."""
...
def _source_uses_cgo(attr, go_infos):
    """Return whether this rule's own sources may need cgo processing."""
...
def maybe_needs_cc_toolchain(attr, go_infos = []):
    """Return whether this rule's own sources may use the C/C++ toolchain."""
    return _source_uses_cgo(attr, go_infos)

def _precomputed_cgo_context_info(attr, go_context_data):
    """Return any precomputed CgoContextInfo provider already attached to the rule."""
...
def go_context(
    ...
    if maybe_needs_cc_toolchain:
        ...
        # Keep cgo tag and build-mode behavior stable for rules that do not use
        # the C/C++ toolchain in their own actions.
