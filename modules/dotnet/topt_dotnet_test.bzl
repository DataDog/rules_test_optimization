...
def _validate_dotnet_test_rule_or_fail(dotnet_test_rule):
    if dotnet_test_rule == None:
        fail_with_prefix("dd_topt_dotnet_test", "you must pass dotnet_test_rule for your selected .NET test rule")

def _select_service_entry_or_fail(topt_data, topt_service):
    return select_service_entry_or_fail(topt_data, topt_service, macro_name = "dd_topt_dotnet_test")
...
def _build_module_labels(sync_repo_name, labels):
    return build_module_labels(sync_repo_name, labels, macro_name = "dd_topt_dotnet_test")
...
def dd_topt_dotnet_test(
    ...
    if not sync_repo_name:
        fail_with_prefix("dd_topt_dotnet_test", "selected topt_data entry is missing required 'repo_name'")
    ...
    raw_name = name + "__raw_dotnet_test"
