"""Macro: dd_topt_dotnet_test.

Wraps a .NET test rule with Datadog Test Optimization wiring and per-module
payload selection.
"""
...
load("//:topt_dotnet_infer.bzl", "topt_dotnet_payloads_selector")
...
def _resolve_topt_service_key(service_entries, topt_service):
    return resolve_topt_service_key(service_entries, topt_service, macro_name = "dd_topt_dotnet_test")

def _validate_dotnet_test_rule_or_fail(dotnet_test_rule):
    if dotnet_test_rule == None:
        fail("dd_topt_dotnet_test: you must pass dotnet_test_rule for your selected .NET test rule")

def _select_service_entry_or_fail(topt_data, topt_service):
    if topt_data == None or not _is_dict(topt_data):
        fail("dd_topt_dotnet_test: topt_data is required and must be the dict from @<repo>//:export.bzl (single-service) or the aggregator mapping")
    ...
    if not service_entries:
        fail("dd_topt_dotnet_test: topt_data mapping did not contain any service entries")
...
validate_dotnet_test_rule_for_tests = _validate_dotnet_test_rule_or_fail
...
def _build_module_labels(sync_repo_name, labels):
    ...
    if not _is_list(labels):
        fail("dd_topt_dotnet_test: selected service topt_data['labels'] must be a list or tuple")

    module_labels = []
    for lab in labels:
        if not _is_string(lab):
            fail("dd_topt_dotnet_test: selected service topt_data['labels'] entries must be strings")
        if not lab:
            fail("dd_topt_dotnet_test: selected service topt_data['labels'] entries must be non-empty")
        for i in range(len(lab)):
            ch = lab[i]
            if ch not in _LABEL_FRAGMENT_ALLOWED_CHARS:
                fail("dd_topt_dotnet_test: selected service topt_data['labels'] entries must be sanitized ([a-z0-9_]): '%s'" % lab)
        module_labels.append("@%s//:module_%s" % (sync_repo_name, lab))
    return module_labels

build_module_labels_for_tests = _build_module_labels

def _build_dotnet_fallback_identifier(package_path, runtime_info):
    pkg_dotted = (package_path or "").replace("\\", ".").replace("/", ".")
    module_path = ((runtime_info or {}).get("module_path") or "")
    if module_path:
        return (module_path + "." + pkg_dotted) if pkg_dotted else module_path
    return pkg_dotted

def dd_topt_dotnet_test(
        name,
        topt_data,
        dotnet_test_rule,
        topt_service = None,
        ...
        **kwargs):
    """Define a .NET test with Datadog Test Optimization support."""
    _validate_dotnet_test_rule_or_fail(dotnet_test_rule)
    ...
    if not sync_repo_name:
        fail("dd_topt_dotnet_test: selected topt_data entry is missing required 'repo_name'")

    _runtimes = _svc.get("runtimes") or {}
    _dotnet = _runtimes.get("dotnet") if _is_dict(_runtimes) else {}
    if not _is_dict(_dotnet):
        _dotnet = {}
    ...
    if uses_inference:
        ...
        module_included = _dotnet.get("module_included") if _is_dict(_dotnet) else None
    ...
    fallback_identifier = _build_dotnet_fallback_identifier(native.package_name(), _dotnet)
