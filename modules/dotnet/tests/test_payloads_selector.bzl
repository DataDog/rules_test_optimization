...
def selector_payload_fixture_targets():
    ...
    _cache_payload(
        name = "module_company_product_cache_tests",
    )
    _dotnet_source(
        name = "deps_leaf",
...
def selector_explicit_miss_failure_target(name, tags = None):
    topt_dotnet_payloads_selector(
        name = name,
        explicit_identifier = "Company.Product.Missing.Tests",
        attribute_candidates = [],
        deps = [],
        fallback_identifier = "Company.Product.Fallback.Tests",
        full_files = ":full_payload",
...
def selector_omits_flaky_tests_target(name, tags = None):
    topt_dotnet_payloads_selector(
        name = name,
        explicit_identifier = "Company.Product.Cache.Tests",
        attribute_candidates = [],
        deps = [],
        fallback_identifier = "Company.Product.Cache.Tests",
        full_files = ":full_payload",
        module_groups = [":module_company_product_cache_tests"],
        include_per_module = True,
        tags = tags,
    )
