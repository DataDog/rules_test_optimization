"""Unit tests for Ruby-specific selection and macro helper functions."""

load("@bazel_skylib//lib:unittest.bzl", "analysistest", "asserts", "unittest")
load(
    "@datadog-rules-test-optimization-ruby//:topt_ruby_infer.bzl",
    "normalize_ruby_identifier_for_tests",
...
load(
    "@datadog-rules-test-optimization-ruby//tests:example_stub_repo.bzl",
...
def _resolve_topt_service_key_prefers_exact_then_sanitized_test(ctx):
    ...
    entries = {
        "ruby_service": {"repo_name": "repo_ruby"},
        "ruby_service_2": {"repo_name": "repo_ruby_2"},
    }
    asserts.equals(env, "ruby_service_2", resolve_topt_service_key_for_tests(entries, "ruby_service_2"))
    asserts.equals(env, "ruby_service", resolve_topt_service_key_for_tests(entries, "ruby-service"))
    return unittest.end(env)

def _select_module_group_name_test(ctx):
    env = unittest.begin(ctx)
    groups = [
        "module_apps_ruby_mod_pkg",
        "module_custom_override",
    ]
    asserts.equals(
        env,
        "module_apps_ruby_mod_pkg",
        select_module_group_name_for_tests("apps/ruby/mod/pkg", groups, True),
    )
    asserts.equals(
        env,
        "module_custom_override",
        select_module_group_name_for_tests("apps/ruby/mod/pkg", groups, True, "custom_override"),
    )
    asserts.equals(
        env,
        "",
        select_module_group_name_for_tests("apps/ruby/mod/pkg", groups, False),
    )
    asserts.equals(
        env,
        "",
        select_module_group_name_for_tests("apps/ruby/other", groups, True),
    )
    return unittest.end(env)

def _normalize_ruby_identifier_edge_cases_test(ctx):
    env = unittest.begin(ctx)
    asserts.equals(env, "apps/ruby/lib", normalize_ruby_identifier_for_tests("  //apps//ruby\\lib// "))
    asserts.equals(env, "apps/ruby/lib", normalize_ruby_identifier_for_tests("apps////ruby///lib"))
    asserts.equals(env, "leading/trailing", normalize_ruby_identifier_for_tests("/leading/trailing/"))
    asserts.equals(env, "", normalize_ruby_identifier_for_tests("////"))
    asserts.equals(env, "", normalize_ruby_identifier_for_tests(None))
    return unittest.end(env)
...
def _ruby_stub_includes_manifest_in_files_test(ctx):
    env = unittest.begin(ctx)
...
normalize_ruby_identifier_edge_cases_test = unittest.make(_normalize_ruby_identifier_edge_cases_test)
...
ruby_stub_includes_manifest_in_files_test = unittest.make(_ruby_stub_includes_manifest_in_files_test)
