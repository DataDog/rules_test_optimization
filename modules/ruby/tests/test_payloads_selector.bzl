...
"""Analysis tests for topt_ruby_payloads_selector selection behavior."""

load("@bazel_skylib//lib:unittest.bzl", "analysistest", "asserts")
load("@datadog-rules-test-optimization-ruby//:topt_ruby_infer.bzl", "topt_ruby_payloads_selector")

_COMMON_MODULE_GROUPS = [
    ":module_apps_ruby_explicit_pkg",
    ":module_apps_ruby_attr_pkg",
    ":module_apps_ruby_deps_pkg",
    ":module_apps_ruby_fallback_pkg",
...
def _ruby_source_impl(_ctx):
    return []

_ruby_source = rule(
    implementation = _ruby_source_impl,
    attrs = {
...
def selector_payload_fixture_targets():
    ...
    _payload_marker(
        name = "module_apps_ruby_explicit_pkg",
        marker = "module:explicit",
    )
    _payload_marker(
        name = "module_apps_ruby_attr_pkg",
        marker = "module:attrs",
    )
    _payload_marker(
        name = "module_apps_ruby_deps_pkg",
        marker = "module:deps",
    )
    _payload_marker(
        name = "module_apps_ruby_fallback_pkg",
    ...
    _cache_payload(
        name = "module_apps_ruby_cache_pkg",
    )
    _ruby_source(
        name = "deps_leaf",
        require_path = "apps/ruby/deps/pkg",
    )
    _ruby_source(
        name = "deps_wrapper",
        deps = [":deps_leaf"],
...
def selector_explicit_precedence_target(name, tags = None):
    topt_ruby_payloads_selector(
        name = name,
        explicit_identifier = "apps/ruby/explicit/pkg",
        attribute_candidates = ["apps/ruby/attr/pkg"],
        deps = [":deps_wrapper"],
        fallback_identifier = "apps/ruby/fallback/pkg",
...
def selector_no_match_fallback_target(name, tags = None):
    topt_ruby_payloads_selector(
        ...
        attribute_candidates = [],
        deps = [],
        fallback_identifier = "apps/ruby/no_match/pkg",
        full_files = ":full_payload",
        module_groups = _COMMON_MODULE_GROUPS,
        include_per_module = True,
        tags = tags,
...
def selector_override_target(name, tags = None):
    topt_ruby_payloads_selector(
        name = name,
        explicit_identifier = "apps/ruby/not_used/pkg",
        attribute_candidates = [],
        deps = [],
        fallback_identifier = "apps/ruby/no_match/pkg",
        full_files = ":full_payload",
...
def selector_explicit_miss_failure_target(name, tags = None):
    topt_ruby_payloads_selector(
        name = name,
        explicit_identifier = "apps/ruby/missing/pkg",
        attribute_candidates = [],
        deps = [],
        fallback_identifier = "apps/ruby/fallback/pkg",
        full_files = ":full_payload",
...
def selector_omits_flaky_tests_target(name, tags = None):
    topt_ruby_payloads_selector(
        name = name,
        explicit_identifier = "apps/ruby/cache/pkg",
        attribute_candidates = [],
        deps = [],
        fallback_identifier = "apps/ruby/cache/pkg",
        full_files = ":full_payload",
        module_groups = [":module_apps_ruby_cache_pkg"],
        include_per_module = True,
        tags = tags,
    )
...
def _selector_explicit_precedence_test_impl(ctx):
    ...
    _assert_selected(env, target, "module_apps_ruby_explicit_pkg")
    return analysistest.end(env)

def _selector_attr_precedence_test_impl(ctx):
    env = analysistest.begin(ctx)
    target = analysistest.target_under_test(env)
    _assert_selected(env, target, "module_apps_ruby_attr_pkg")
    return analysistest.end(env)

def _selector_deps_precedence_test_impl(ctx):
    env = analysistest.begin(ctx)
    target = analysistest.target_under_test(env)
    _assert_selected(env, target, "module_apps_ruby_deps_pkg")
    return analysistest.end(env)

def _selector_fallback_test_impl(ctx):
    env = analysistest.begin(ctx)
    target = analysistest.target_under_test(env)
    _assert_selected(env, target, "module_apps_ruby_fallback_pkg")
    return analysistest.end(env)
