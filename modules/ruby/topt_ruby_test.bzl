"""Macro: dd_topt_ruby_test.

Wraps a Ruby test rule with Datadog Test Optimization wiring and per-module
payload selection.
"""
...
load("//:topt_ruby_infer.bzl", "topt_ruby_payloads_selector")
...
def _resolve_topt_service_key(service_entries, topt_service):
    return resolve_topt_service_key(service_entries, topt_service, macro_name = "dd_topt_ruby_test")

def _validate_ruby_test_rule_or_fail(ruby_test_rule):
    if ruby_test_rule == None:
        fail("dd_topt_ruby_test: you must pass ruby_test_rule for your selected Ruby test rule")

def _select_service_entry_or_fail(topt_data, topt_service):
    if topt_data == None or not _is_dict(topt_data):
        fail("dd_topt_ruby_test: topt_data is required and must be the dict from @<repo>//:export.bzl (single-service) or the aggregator mapping")
    ...
    if not service_entries:
        fail("dd_topt_ruby_test: topt_data mapping did not contain any service entries")
...
validate_ruby_test_rule_for_tests = _validate_ruby_test_rule_or_fail
...
def _build_module_labels(sync_repo_name, labels):
    ...
    if not _is_list(labels):
        fail("dd_topt_ruby_test: selected service topt_data['labels'] must be a list or tuple")

    module_labels = []
    for lab in labels:
        if not _is_string(lab):
            fail("dd_topt_ruby_test: selected service topt_data['labels'] entries must be strings")
        if not lab:
            fail("dd_topt_ruby_test: selected service topt_data['labels'] entries must be non-empty")
        for i in range(len(lab)):
            ch = lab[i]
            if ch not in _LABEL_FRAGMENT_ALLOWED_CHARS:
                fail("dd_topt_ruby_test: selected service topt_data['labels'] entries must be sanitized ([a-z0-9_]): '%s'" % lab)
        module_labels.append("@%s//:module_%s" % (sync_repo_name, lab))
    return module_labels

build_module_labels_for_tests = _build_module_labels

def _build_ruby_fallback_identifier(package_path, runtime_info):
...
def dd_topt_ruby_test(
        ...
        **kwargs):
    """Define a Ruby test with Datadog Test Optimization support."""
    _validate_ruby_test_rule_or_fail(ruby_test_rule)
    ...
    if not sync_repo_name:
        fail("dd_topt_ruby_test: selected topt_data entry is missing required 'repo_name'")

    _runtimes = _svc.get("runtimes") or {}
    _ruby = _runtimes.get("ruby") if _is_dict(_runtimes) else {}
    if not _is_dict(_ruby):
        _ruby = {}
    ...
    if uses_inference:
        ...
        module_included = _ruby.get("module_included") if _is_dict(_ruby) else None
    ...
    fallback_identifier = _build_ruby_fallback_identifier(native.package_name(), _ruby)

    selector_name = name + "_topt_payloads"
    topt_ruby_payloads_selector(
        name = selector_name,
        deps = deps_labels,
