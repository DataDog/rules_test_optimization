...
def selector_payload_fixture_targets():
    ...
    _cache_payload(
        name = "module_packages_nodejs_cache_pkg",
    )
    _nodejs_source(
        name = "deps_leaf",
...
def selector_omits_flaky_tests_target(name, tags = None):
    topt_nodejs_payloads_selector(
        name = name,
        explicit_identifier = "packages/nodejs/cache/pkg",
        attribute_candidates = [],
        deps = [],
        fallback_identifier = "packages/nodejs/cache/pkg",
        full_files = ":full_payload",
        module_groups = [":module_packages_nodejs_cache_pkg"],
        include_per_module = True,
        tags = tags,
    )
