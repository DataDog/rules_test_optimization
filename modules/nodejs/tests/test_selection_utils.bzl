"""Unit tests for NodeJS-specific selection and macro helper functions."""

load("@bazel_skylib//lib:unittest.bzl", "analysistest", "asserts", "unittest")
load(
    "@datadog-rules-test-optimization-nodejs//:topt_nodejs_infer.bzl",
    "normalize_nodejs_identifier_for_tests",
...
load(
    "@datadog-rules-test-optimization-nodejs//tests:example_stub_repo.bzl",
...
def _service_mapping_entries_filters_non_service_test(ctx):
    ...
    asserts.equals(env, "repo_nodejs", entries["nodejs_service"]["repo_name"])
...
def _resolve_topt_service_key_prefers_exact_then_sanitized_test(ctx):
    ...
    entries = {
        "nodejs_service": {"repo_name": "repo_nodejs"},
        "nodejs_service_2": {"repo_name": "repo_nodejs_2"},
    }
    asserts.equals(env, "nodejs_service_2", resolve_topt_service_key_for_tests(entries, "nodejs_service_2"))
    asserts.equals(env, "nodejs_service", resolve_topt_service_key_for_tests(entries, "nodejs-service"))
    return unittest.end(env)

def _select_module_group_name_test(ctx):
    env = unittest.begin(ctx)
    groups = [
        "module_packages_nodejs_mod_pkg",
        "module_custom_override",
    ]
    asserts.equals(
        env,
        "module_packages_nodejs_mod_pkg",
        select_module_group_name_for_tests("packages/nodejs/mod/pkg", groups, True),
    )
    asserts.equals(
        env,
        "module_custom_override",
        select_module_group_name_for_tests("packages/nodejs/mod/pkg", groups, True, "custom_override"),
    )
    asserts.equals(
        env,
        "",
        select_module_group_name_for_tests("packages/nodejs/mod/pkg", groups, False),
    )
    asserts.equals(
        env,
        "",
        select_module_group_name_for_tests("packages/nodejs/other", groups, True),
    )
    return unittest.end(env)

def _normalize_nodejs_identifier_edge_cases_test(ctx):
    env = unittest.begin(ctx)
    asserts.equals(env, "pkg/sub/mod", normalize_nodejs_identifier_for_tests("  //pkg//sub\\mod// "))
    asserts.equals(env, "pkg/sub/mod", normalize_nodejs_identifier_for_tests("pkg////sub///mod"))
    asserts.equals(env, "leading/trailing", normalize_nodejs_identifier_for_tests("/leading/trailing/"))
    asserts.equals(env, "", normalize_nodejs_identifier_for_tests("////"))
    asserts.equals(env, "", normalize_nodejs_identifier_for_tests(None))
    return unittest.end(env)
...
def _nodejs_stub_includes_manifest_in_files_test(ctx):
    env = unittest.begin(ctx)
...
normalize_nodejs_identifier_edge_cases_test = unittest.make(_normalize_nodejs_identifier_edge_cases_test)
...
nodejs_stub_includes_manifest_in_files_test = unittest.make(_nodejs_stub_includes_manifest_in_files_test)
