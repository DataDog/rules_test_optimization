...
if /I "%%PAYLOADS_IN_FILES%%"=="true" if not "%%UNDECLARED_DIR%%"=="" (
  ...
  powershell -NoProfile -Command "$portFile = [string]$env:CAPTURE_PORT_FILE; for ($i = 0; $i -lt 100; $i++) { if (Test-Path -LiteralPath $portFile -PathType Leaf) { $port = (Get-Content -LiteralPath $portFile -Raw -ErrorAction SilentlyContinue).Trim(); if ($port -match '^[0-9]+$') { exit 0 } }; Start-Sleep -Milliseconds 100 }; exit 1" >nul
  if errorlevel 1 (
    echo orch_go_test: CI Visibility capture helper failed to publish a numeric port file 1>&2
    exit /b 1
  )
